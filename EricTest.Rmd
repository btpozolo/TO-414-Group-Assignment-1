...
M1$starttime <- as.POSIXct(as.character(M1$starttime), format = '%m/%d/%Y %H:%M:%S')
M1$stoptime <- as.POSIXct(as.character(M1$stoptime), format = '%m/%d/%Y %H:%M:%S')
...
M10$starttime <- as.POSIXct(as.character(M10$starttime), format = '%Y-%m-%d %H:%M:%S')
M10$stoptime <- as.POSIXct(as.character(M10$stoptime), format = '%Y-%m-%d %H:%M:%S')
...
```{r}
#By Month
trainingdata$month <- month(trainingdata$month)
...
#map
ggmap(NYCmap) + 
  geom_point(aes(x=station.longitude,y=station.latitude, fill = netflow,color = sur.or.def, alpha = netflow, size = netflow/10), data= trip.total.defsur)+ggtitle("Top 50 Deficit and Top 50 Surplus Stations of 2016")
...
#map
ggmap(NYCmap) + 
  geom_point(aes(x=station.longitude,y=station.latitude, fill = netflow,color = sur.or.def, alpha = netflow, size = netflow/10), data= trip.by.jan.defsur)+ggtitle("Top 50 Deficit and Top 50 Surplus Stations of January 2016")
...
#map
ggmap(NYCmap) + 
  geom_point(aes(x=station.longitude,y=station.latitude, fill = netflow,color = sur.or.def, alpha = netflow, size = netflow/10), data= trip.by.weekend.defsur)+ggtitle("Top 50 Deficit and Top 50 Surplus Stations of 2016 (weekend)")
...
#map
ggmap(NYCmap) + 
  geom_point(aes(x=station.longitude,y=station.latitude, fill = netflow,color = sur.or.def, alpha = netflow, size = netflow/10), data= trip.by.weekday.defsur)+ggtitle("Top 50 Deficit and Top 50 Surplus Stations of 2016 (weekday)")
```
...
```
The above histogram shows that the data shows more of a somewhat normal distribution of usage, which could indicate that there is not reason to believe that there are other factors outside of chance causing certain bikes to be used more. 
